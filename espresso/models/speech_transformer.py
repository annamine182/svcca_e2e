    ...
    *args.encoder_layers* layers. Each layer is a :class:`TransformerEncoderLayer`.
    ...
    def forward(
        ...
        for layer in self.layers:
            ...
            #Anna embedding extraction
            y=x 
            emb = {}
            emb[ str(counter)+"fc1" ]= y
            global svcca_path
            svcca_path = "/share/mini1/sw/spl/espresso/new_svcca/svcca/exp/tmp_embeddings/"
            filename = str(counter)+"fc1_cuda.pkl"
            ...
            y=x
            emb = {}
            emb[ str(counter)+"fc2" ]=y
            filename = str(counter)+"fc2_cuda.pkl"
            print("Final embedding shape at {} is {}".format(filename,y.shape))
            filename = os.path.join(svcca_path, filename)
            torch.save(emb,filename)
            y=x
            emb = {}
            emb[ str(counter)+"final_layer_norm" ]=y
            filename = str(counter)+"final_layer_norm_cuda.pkl"
            print("Final embedding shape at {} is {}".format(filename,y.shape))
            filename = os.path.join(svcca_path, filename)
            torch.save(emb,filename)
            y=x
            emb = {}
            emb[ str(counter)+".self_attn.out_proj" ]=y
            filename = str(counter)+".self_attn.out_proj_cuda.pkl"
            print("Final embedding shape at {} is {}".format(filename,y.shape))
            filename = os.path.join(svcca_path, filename)
            torch.save(emb,filename)
            counter = counter+1
