...
def cca_conv(embedding_files,keys):
  ...
  for f_index in range(len(embedding_files)-1):
    ...
    #layerA = torch.isnan(layerA).any()
    #layerB = torch.isnan(layerB).any()
  ...
  np.save('/share/mini1/sw/spl/espresso/new_svcca/svcca/exp/svcca/results/{0}/{1}.npy'.format(file_name,keys),results)
  ...
  #np.save('/share/mini1/sw/spl/espresso/new_svcca/svcca/exp/svcca/results/{0}/_roc{1}.npy'.format(filename,keys),results_0)
...
def cca_lstm(embedding_files,keys):
  ...
  for f_index in range(len(embedding_files)-1):
    ...
    pwcca_similarity, w, _ = compute_pwcca(layerA.cpu().T.detach().numpy(), layerB.cpu().T.detach().numpy(), epsilon=1e-10)
    results_pwcca.append(pwcca_similarity)
    results_feat.append(cka_from_features)
  np.save('/share/mini1/sw/spl/espresso/new_svcca/svcca/exp/svcca/results/{0}/{1}.npy'.format(file_name,keys),results)
  np.save('/share/mini1/sw/spl/espresso/new_svcca/svcca/exp/svcca/results_cka/{0}/{1}.npy'.format(file_name,keys),results_feat)
  np.save('/share/mini1/sw/spl/espresso/new_svcca/svcca/exp/svcca/results/pwcca_ep/{0}/{1}.npy'.format(file_name,keys),results_pwcca)
  ...
  #  results_0.append(np.mean(similarity["cca_coef1"]))
  #  print("cca value for 0th to {}th is {} ".format(f_index+1,results_0))
  #np.save('/share/mini1/sw/spl/espresso/new_svcca/svcca/exp/svcca/results/{0}/roc_{1}.npy'.format(filename,subkeys),results_0))
...
def cca_trans(embedding_files,subkeys):
  ...
  for f_index in range(len(embedding_files)-1):
    ...
    cka_from_features = feature_space_linear_cka(layerA.cpu().detach().numpy(),layerB.cpu().detach().numpy())
    results_cka.append(cka_from_features)
    pwcca_similarity, w, _ = compute_pwcca(layerA.cpu().T.detach().numpy(), layerB.cpu().T.detach().numpy(), epsilon=1e-10)
    results_pwcca.append(pwcca_similarity)
  np.save('/share/mini1/sw/spl/espresso/new_svcca/svcca/exp/svcca/results/{0}/{1}.npy'.format(file_name,subkeys),results)
  np.save('/share/mini1/sw/spl/espresso/new_svcca/svcca/exp/svcca/results_cka/{0}/{1}.npy'.format(file_name,subkeys),results_cka)
  np.save('/share/mini1/sw/spl/espresso/new_svcca/svcca/exp/svcca/results/pwcca_ep/{0}/{1}.npy'.format(file_name,subkeys),results_pwcca)
...
def cka_conv(embedding_files,keys):
  ...
  for f_index in range(len(embedding_files)-1):
    ...
    layerB = layerB.reshape((b1*f1*d1,c1))
    print("net1 ={} net2={}".format(layerA.shape,layerB.shape))
    #cka_from_examples = cka(gram_linear(layerA.cpu().T.detach().numpy()), gram_linear(layerB.cpu().T.detach().numpy()))
    cka_from_features = feature_space_linear_cka(layerA.cpu().detach().numpy(),layerB.cpu().detach().numpy())
    ...
    print("cka features: ", results_feat)
    #print("rbf kernel: ", rbf)
  np.save('/share/mini1/sw/spl/espresso/new_svcca/svcca/exp/svcca/results_cka/{0}/{1}.npy'.format(file_name,keys),results_feat)

def pwcca_conv(embedding_files,keys):
  # cca convolutions change for new format
  # BATCH X NUMBER OF FRAMES X (DIMENSIONALITY * FEATURE SIZE) X CHANNEL
  results = []
  for f_index in range(len(embedding_files)-1):
    ...
    print("net1 ={} net2={}".format(layerA.shape,layerB.shape))
    pwcca_similarity, w, _ = compute_pwcca(layerA.cpu().T.detach().numpy(), layerB.cpu().T.detach().numpy(), epsilon=1e-10)
    results.append(pwcca_similarity)
    print("pwcca value: ", results)
  np.save('/share/mini1/sw/spl/espresso/new_svcca/svcca/exp/svcca/results/pwcca_ep/{0}/{1}.npy'.format(file_name,keys),results)
...
def main_fct(root_dir,keys,subkeys):
  ...
  if "conv" in keys:
      ...
      #pwcca_c = pwcca_conv(embedding_files,keys)
  elif "lstm" in keys:
      print("LSTM analysis...")
